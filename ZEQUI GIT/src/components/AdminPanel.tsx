...
import {
  subscribeToOrders, updateOrderStatus, deleteAllOrders,
...
function OrdersTab() {
  ...
  const canClearAll = orders.length > 0 && !clearingAll;

  const handleClearAll = async () => {
    if (!canClearAll) return;
    const ok = confirm("¿Eliminar TODOS los pedidos? Esta acción no se puede deshacer.");
    if (!ok) return;
    setClearingAll(true);
    try {
    ...
    <div className="space-y-6 pb-24 md:pb-6">
      ...
      <div className="hidden md:block">
        <button
          ...
          className="inline-flex items-center gap-2 bg-red-700 hover:bg-red-600 disabled:bg-white/10 disabled:text-white/30 text-white font-bold px-4 py-2 rounded-xl text-sm transition-colors"
        >
          <Trash2 size={14} />
          {clearingAll ? "Eliminando..." : "Eliminar Todos Los Pedidos"}
        </button>
      </div>
      ...
      {/* Mobile/PWA floating action */}
      <button
        onClick={handleClearAll}
        disabled={!canClearAll}
        className="md:hidden fixed left-4 right-4 z-[1000] rounded-2xl px-4 py-3 font-black text-sm tracking-wide bg-red-700 text-white shadow-[0_14px_40px_rgba(120,0,0,0.45)] border border-red-500/60 disabled:bg-white/10 disabled:text-white/30 disabled:border-white/10"
        style={{ bottom: "calc(env(safe-area-inset-bottom) + 16px)" }}
      >
        {clearingAll ? "Eliminando pedidos..." : "Eliminar Todos Los Pedidos"}
      </button>
    </div>
  );
}
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      // Reemplazar coma por punto (teclados móviles latinos)
                      val = val.replace(/,/g, '.');
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div><label className={LABEL}>TikTok handle</label><input value={data.tiktokHandle || ""} onChange={f("tiktokHandle")} placeholder="@zequismash" className={INPUT} /></div>
        <div><label className={LABEL}>TikTok URL</label><input value={data.tiktokUrl || ""} onChange={f("tiktokUrl")} placeholder="https://tiktok.com/@zequismash" className={INPUT} /></div>
...
export default function AdminPanel() {
  ...
  return (
    <div className="min-h-dvh bg-[#050505] overflow-x-hidden">
      <div
        className="bg-[#0a0a0a] border-b border-white/10 px-4 md:px-6 py-4 flex items-center justify-between sticky z-40"
      ...
      <div className="border-b border-white/5 px-4 md:px-6 bg-[#080808] overflow-x-auto">
