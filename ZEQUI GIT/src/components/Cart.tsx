...
function safePrice(price: string, qty: number = 1): string {
...
export default function Cart({ items, total, open, onOpen, onClose, onUpdateQty, onCheckout }: CartProps) {
  ...
  return (
    <>
      ...
      {/* X button — always on top, independent of drawer */}
      <AnimatePresence>
        {open && (
          <motion.button
            ...
            onClick={onClose}
            className="fixed z-[99999] bg-[#8B0000] text-white rounded-full flex items-center justify-center shadow-2xl border-2 border-white"
            style={{
      ...
      <AnimatePresence>
        {open && (
          ...
          >
            {/* Header */}
            <div className="flex items-center justify-between px-6 py-4 border-b border-white/10 mt-14">
              <h2 className="text-white font-black text-2xl">Tu Pedido</h2>
              <span className="text-white/30 text-sm">{count} {count === 1 ? "item" : "items"}</span>
            </div>
                ...
                items.map((item) => (
                  <div key={item.id} className="flex items-center gap-4 bg-white/[0.03] rounded-2xl p-4 border border-white/5">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[#FF6B00] font-black text-lg">${safePrice(item.price, item.quantity)}</p>
                    </div>
                    <div className="flex items-center gap-2">
                      <button
                        onClick={() => onUpdateQty(item.id, -1)}
                        className="w-9 h-9 rounded-full bg-white/10 hover:bg-[#8B0000] text-white flex items-center justify-center transition-colors"
                      >
            ...
            {items.length > 0 && (
              <div className="p-6 border-t border-white/10 space-y-4">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-white font-black text-3xl">${isNaN(total) ? "0.00" : total.toFixed(2)}</span>
                </div>
                <button
                  onClick={onCheckout}
