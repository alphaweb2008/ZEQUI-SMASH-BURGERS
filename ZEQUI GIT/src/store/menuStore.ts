...
export const DEFAULT_CATEGORIES = ["Burgers", "Bebidas", "Alitas", "Papas", "Combos"];
...
// NO llama callback con defaults primero — solo usa lo que viene de Firestore
  ...
  return onSnapshot(
    ...
    (snap) => {
      if (snap.exists()) {
        ...
        // Si no existe el documento, crearlo SOLO la primera vez con defaults
        setDoc(doc(db, CATS_DOC), { list: DEFAULT_CATEGORIES }).catch(() => {});
        callback(DEFAULT_CATEGORIES);
      }
    },
    () => {
      // Si hay error de Firestore, NO sobreescribe — usa lo que hay
      callback([]);
    }
  );
}
