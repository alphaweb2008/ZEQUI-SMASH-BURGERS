...
function MenuTab() {
  ...
  const handleSave = async () => {
    ...
    const finalForm = { ...form, price: priceInput.trim() };
    setSaving(true);
    try {
      if (editId) {
            ...
            <div className="bg-white/[0.03] border border-[#8B0000]/30 rounded-2xl p-6 space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input type="number" value={priceInput} onChange={(e) => setPriceInput(e.target.value)} placeholder="Ej: 8.50" min="0" step="0.01" className={INPUT} />
                </div>
                <div>
          ...
          <div key={item.id} className="bg-white/[0.03] border border-white/10 rounded-2xl overflow-hidden">
            ...
            <div className="p-4">
              <div className="flex justify-between items-start mb-1">
                ...
                <p className="text-[#FF6B00] font-black text-sm ml-2">${parseFloat(item.price || "0").toFixed(2)}</p>
              </div>
